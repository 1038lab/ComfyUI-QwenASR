...
def _load_cached_aligner(
  ...
  """Load and cache Qwen3ForcedAligner instance."""
  if Qwen3ForcedAligner is None:
    raise RuntimeError(f"Qwen3ForcedAligner not available: {_IMPORT_ERROR}")

  key = (aligner_path, str(dtype), str(device), attention)
  cached = _ALIGNER_MODEL_CACHE.get(key)
  if cached is not None:
    return cached
  ...
  aligner = Qwen3ForcedAligner.from_pretrained(aligner_path, **kwargs)
  _ALIGNER_MODEL_CACHE[key] = aligner
  return aligner
...
class AILab_Qwen3ASRToSRT:
  ...
  def INPUT_TYPES(cls):
    ...
    return {
        "required": {
            "audio": ("AUDIO", {"tooltip": "Audio input for alignment."}),
            "original_text": ("STRING", {"forceInput": True, "tooltip": "Original text to align with audio timestamps."}),
        },
        "optional": {
            ...
            "precision": (["bf16", "fp16", "fp32"], {"default": defaults.get("precision", "bf16"), "tooltip": "Inference precision."}),
            "language": (SUPPORTED_LANGUAGES, {"default": defaults.get("language", "auto"), "tooltip": "Language for text tokenization."}),
  ...
  def to_srt(
    ...
    if not forced_aligner or forced_aligner == "None":
      raise ValueError("forced_aligner is required for SRT generation")
    ...
    aligner = _load_cached_aligner(aligner_path, dtype, device, "auto")
